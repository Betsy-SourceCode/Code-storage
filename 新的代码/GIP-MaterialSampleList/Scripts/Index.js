...
function IndexList() {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.length == 0) {
                ...
                $.each(data, function (key, value) {
                    if (m.has(value["流程编号"]) && value["流程状态"] != "5") {  //has某个键是否在当前 Map 对象
                        ...
                        approvalStepNode.timeConsumingWork = GettimeConsumingWork(approvalStepNode.submitTime, approvalStepNode.approveTime, approvalStepNode.timeConsumingWork);
                    ...
                    if (value.FlowStatus == $("#Status option:selected").text() || $("#Status").val() == -1) {
                        for (var i = 0; i < value.ApprovalStepNodes.length; i++) {
                            for (var j = 0; j < value.ApprovalStepNodes[i].length; j++) {
                                ...
                                Worktime = Worktime += value.ApprovalStepNodes[i][j].timeConsumingWork;

                            }
                        }
                        //console.log(TimeCount);
                        addtr(value, xh, TimeCount, Worktime);
                        xh++;
                    };
            ...
            //加载完成后  关闭遮罩层
            $("#myModal").modal('hide');
            $('.freeze-table').freezeTable({ 'scrollBar': true, 'shadow': true, 'columnNum': 13, 'freezeColumn': true,'width':'100%' });//冻结
...
/**
 * 获得工作耗时转换为小时
 * @param getsubmitTime    接收时间
 * @param getapproveTime  审批时间
 * @param diffDay    接收时间-审批时间之间的天数差
 * @param timeConsumingWork   工作耗时  分钟
 */
function GettimeConsumingWork(getsubmitTime, getapproveTime, timeConsumingWork) {
    var submitTime = new Date(getsubmitTime);
    var approveTime = new Date(getapproveTime);
    var start = formatDateTime(submitTime);
    var end = formatDateTime(approveTime);
    var s = new Date(start);
    var e = new Date(end);
    var diffDay = 0;
    if (s.getFullYear() != e.getFullYear() && s.getMonth() != e.getMonth() && s.getDate() != e.getDate()) {
        diffDay = (s - e) / (1000 * 60 * 60 * 24) + 1;
    }

    for (var f = 0; f < diffDay; f++) {
        if (submitTime.getDay() == 0 || submitTime.getDay() == 6 || approveTime.getDay() == 0 || approveTime
            .getDay() == 6) {
            timeConsumingWork = parseInt(timeConsumingWork - 480); //减掉周末的24个小时
        }
        submitTime.setDate(submitTime.getDate() + 1);
    }
    timeConsumingWork = (Math.floor((timeConsumingWork / 60) * 100) / 100).toString();
    return parseFloat(timeConsumingWork);
}
function addtr(value, xh, ActualTimeCount, WorkTimeCount) {
    var tr = "";
    ...
    var ActualTime = (Math.floor((ActualTimeCount / 60) * 100) / 100).toString();//实际耗时
                ...
                switch (j) {
                    ...
                    case 8:
                        if (value["ApprovelContent"].length > 28) {
                            if (flowcount >=3) {
                                var ApprovelContent = value["ApprovelContent"];
                    ...
                    case 10:
                        ...
                        break
                    case 11:
                        tr += '<td>' + Math.round(WorkTimeCount * 100) / 100 + '</td>'
                        break
                    default:
                        tr += '<td></td>'
                }
                    ...
                    for (var k = 0; k <= value["ApprovalStepNodes"][i].length; k++) {
                        if (value["ApprovalStepNodes"][i][k] == undefined) {   //步骤很少，没走完整流程,不做判断会报错
                            ...
                            if (stepName[step] == value["ApprovalStepNodes"][i][k]["stepNodeName"]) {  //遍历所有节点找出和当前数据节点对应的节点
                                ...
                                //工作耗时超过16小时  字体红色
                                if (value["ApprovalStepNodes"][i][k]["timeConsumingWork"] > 16) {
                                    tr += '<td class="fontred">' + value["ApprovalStepNodes"][i][k]["timeConsumingWork"] + '</td>'; //工作耗时
                                } else {
                                    tr += '<td>' + value["ApprovalStepNodes"][i][k]["timeConsumingWork"] + '</td>'; //工作耗时
                                }
                                //tr += '<td>' + (Math.floor((value["ApprovalStepNodes"][i][k]["timeConsuming"] / 60) * 100) / 100).toString() + '</td>';
                                break;
                            }
                        }
    ...
    //mergeCell("GridView", startrow, endrow, 15);
    mergeCell("GridView", startrow, endrow, 15);
...
function Download(obj) {
    ...
    //当从冻结表格span符号点击进入时，obj就等于冻结表格的span
    //当从主表格span符号进入时，obj就等于主表格的span
    ...
    var tr = "";
    var trindex = $('tr').index($(obj).parent());
    var frozenspan = $(".clone-column-table-wrap").find("table tr").eq(trindex).children().children().children(); //获取冻结表格中当前行的span
      if (spanObj.hasClass("glyphicon-plus")) {    //如果是+改成-/如果是-改成+
        spanObj.removeClass("glyphicon-plus");
          spanObj.addClass("glyphicon-minus");
          //设置冻结表格中span的符号与主表格中当前行的span符号保持一致
          frozenspan.removeClass("glyphicon-plus");
          frozenspan.addClass("glyphicon-minus");
        $("#myModal").modal({ backdrop: 'static', keyboard: false });
        //通过Main获取路径
        $.ajax({
            ...
            success: function (data) {
                ...
                if (index == 0) {
                    $(obj).parent().after(tr);
                    $(".clone-column-table-wrap").find("table tr").eq(trindex).after(tr);
                } else {
        ...
        spanObj.addClass("glyphicon-plus");
          if (spanObj.hasClass("glyphicon-plus")) {//如果冻结表格的span符号为+ 
              //主表格的span符号也为+
            $(".freeze-table #GridView tbody").find("tr").children().eq(0).children().children().removeClass("glyphicon-minus");
            $(".freeze-table #GridView tbody").find("tr").children().eq(0).children().children().addClass("glyphicon-plus");
        } 
        //移除二级表格的tr
