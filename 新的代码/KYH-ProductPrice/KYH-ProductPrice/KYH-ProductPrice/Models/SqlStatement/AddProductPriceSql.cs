...
namespace KYH_ProductPrice.Models.SqlStatement
{
	...
	public class AddProductPriceSql
	{
		...
		{
			string sql = "SELECT  A.CustomerID AS ID,ISNULL(B.FullName_EN, B.FullName_CN) + ' (' + C.CountryID + '-' + C.CityName_CN + ')' AS Text FROM Customer AS A LEFT OUTER JOIN Company AS B ON A.SysID = B.CompanyID LEFT OUTER JOIN City AS C ON B.CityID = C.CityID WHERE (B.InBusiness = 'N') AND (B.Agreement = 1) AND (A.ServiceBy LIKE '|%"+LoginUserName+"%|') ORDER BY Text";
			return this.db.Database.SqlQuery<Others>(sql, new object[0]).ToList<Others>();
		}
