...
namespace MIS_CertificationApplication.Models
{
    ...
    public class Authority
    {
        ...
        public string GetDepartmentSql(string EmpID, int num)
        {
            try
            {
                Others list = this.db.Database.SqlQuery<Others>("select UpperDept + D.DeptID AS Text,d.DeptCode AS Name from Department d,Employee e where d.deptid=e.deptid and EmpID=@EmpID", new SqlParameter("@EmpID", EmpID)).FirstOrDefault<Others>();
                string result = "";
                if (list == null)
                {
                    return "";
