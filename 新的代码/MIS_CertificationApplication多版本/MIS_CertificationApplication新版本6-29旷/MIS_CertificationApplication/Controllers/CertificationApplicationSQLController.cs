...
namespace MIS_CertificationApplication.Controllers
{
    public class CertificationApplicationSQLController : Controller
    {
        ...
        {
            public NewCertificates_Master datalist;
            /// <summary>
            /// 权限：‘A’= 全能、‘B’= 维护、‘C’= 基本
            /// </summary>
            public string RLV;
        ...
        public string GetCertificatesManagementList(string CountryAreas, string key_words, string Cancel)
        {
            try
            {
                ...
                {
                    newlist.Add(new NEWSelectIndex { datalist = item, RLV = new Authority().GetRLV(Session["userid"].ToString(), item.CreateBy, item.CreateDept) });
                }
