        ...
        public static int AddLog_ErrorData(int code, string ERPMO, string msg, DateTime dt, int count, string typeName)
        {
            try
            {
                lock (locker1)//加线程锁，防止并发
                {
                    ...
                    if (code == 1100)
                    {
                        log.Error_Reason = "自动模式下正确代码为：" + code + "，" + typeName + "接口返回：" + msg + ",实际返回：" + count + "条";
                    }
                    else
                    {
                        log.Error_Reason = "自动模式下错误代码为：" + code + "，" + typeName + "接口返回：" + msg;
                    }
                    if (code == 0 || code == 1) //0-程序进入登录后报错，1-程序登录报错
                    {
                        log.Error_Reason = "自动模式下错误代码为：" + code + "，" + typeName + "程序返回：" + msg;
                    }
