...
namespace Login
{
    public partial class Details : Form
    {
        public Details()
        {
            Control.CheckForIllegalCrossThreadCalls = false; //不加这一句异步会报错
            InitializeComponent();
        }
        ...
        private void Details_Load(object sender, EventArgs e)
        {
            ...
            dataGridView1.RowTemplate.Height = 10; //改变行的高度;
            dataGridView1.DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;//行内容居中
            dataGridView1.EnableHeadersVisualStyles = false;//这一句很重要，否则下面的列头设置不起作用
            dataGridView1.ColumnHeadersDefaultCellStyle.BackColor = Color.LightBlue;//表头背景颜色
            ...
            //this.dataGridView1.DataSource = this.listSource;
            //委托事件添加具体事件
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void Factory_processes_skinButton_ClickAsync(object sender, EventArgs e)
        {
            ...
            if (this.Factory_processes_ucComboxGrid.TextValue != "点我看工单号列表")
            {
                //出现等待蒙版
                //设置异步，解决页面卡死
                MaskingExt.Show(this, new MaskingExt.MaskingSettings() { TextOrientation = MaskingExt.MaskingTextOrientations.Right, Text = "执行中..." });//执行的方法示例这里用延迟代替
                ToDo();
            }
            else
            {
        ...
        {
            this.dataGridView1.DataSource = currentSource;
        }
        /// <summary>
        /// 表格页签-初始化表格数据源
        /// </summary>
        private void initDataSource()
        {
            ...
            //var data1 = data.OrderByDescending(a => a.Error_Date
            //).Skip((1 - 1) * 10).Take(10).ToList();//分页
            this.dataGridView1.DataSource = this.listSource.Skip((1 - 1) * 10).Take(10).ToList();
        ...
        private void ucDataGridView1_ItemClick(object sender, DataGridViewEventArgs e)
        {
            ...
            //委托事件添加具体事件
            var aa = dataGridView1.SelectedRows[e.RowIndex].ToString();
